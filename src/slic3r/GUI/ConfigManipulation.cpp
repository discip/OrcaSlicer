...
void ConfigManipulation::toggle_print_fff_options(DynamicPrintConfig *config, const bool is_global_config)
{
    ...
    for (auto el : { "top_surface_line_width", "top_surface_speed" })
        toggle_line(el, has_top_solid_infill || (has_spiral_vase && has_bottom_solid_infill));
    ...
    // BBS
    //toggle_line("support_material_synchronize_layers", have_support_soluble);
    ...
    toggle_line("seam_slope_min_length", !config->opt_bool("seam_slope_entire_loop"));
